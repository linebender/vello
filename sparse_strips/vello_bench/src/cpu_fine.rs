...
use vello_cpu::paint::{Stop, EncodedPaint, LinearGradient, SweepGradient};
...
pub fn fill(c: &mut Criterion) {
    ...
    fill_single!(opaque, &Paint::Solid(ROYAL_BLUE.premultiply().to_rgba8()), &[]);
    fill_single!(transparent, &Paint::Solid(ROYAL_BLUE.with_alpha(0.2).premultiply().to_rgba8()), &[]);
...
pub fn strip(c: &mut Criterion) {
    ...
    strip_single!(basic, &Paint::Solid(ROYAL_BLUE.premultiply().to_rgba8()), &[]);
