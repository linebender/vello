    ...
    pub(super) fn blend<S: Simd, T: Iterator<Item = u8x32<S>>>(
        ...
        for (next_dest, next_src) in dest.chunks_exact_mut(32).zip(src) {
            ...
            let src_v = if default_mix {
                ...
                blend::mix(simd, next_src, bg_v, blend_mode)
            };
...
mod alpha_fill {
    ...
    pub(super) fn blend<S: Simd, T: Iterator<Item = u8x32<S>>>(
        ...
        for ((next_bg, next_mask), next_src) in dest.chunks_exact_mut(32).zip(alphas).zip(src) {
    ...
    pub(super) fn alpha_composite<S: Simd, T: Iterator<Item = u8x32<S>>>(
        ...
        for ((next_dest, next_mask), next_src) in dest.chunks_exact_mut(32).zip(alphas).zip(src) {
    ...
    fn pack_block_unpack_block_roundtrip() {
        ...
        dispatch!(Level::try_detect().unwrap_or(Level::baseline()), simd => {
            test_pack_unpack_roundtrip(
                width,
                |region, scratch| {
                    pack_block(simd, scratch, usize::from(width), region);
                },
                |region, scratch| {
                    unpack_block(simd, region, usize::from(width), scratch);
                },
            );
        });
    }
    ...
    fn pack_unpack_roundtrip() {
        ...
        dispatch!(Level::try_detect().unwrap_or(Level::baseline()), simd => {
            test_pack_unpack_roundtrip(
                width,
                |region, scratch| {
                    pack(simd, scratch, usize::from(width), region);
                },
                |region, scratch| {
                    unpack(simd, region, usize::from(width), scratch);
                },
            );
        });
    }
