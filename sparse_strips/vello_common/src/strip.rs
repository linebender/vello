...
fn render_impl<S: Simd>(
    ...
    for (occupied_row_idx, row) in tiles.occupied_tile_rows().enumerate() {
        ...
        let row = row as u16;
        let tile_row = tiles.get_tile_row(row);
        let tile_top_y = f32::from(row) * f32::from(Tile::HEIGHT);
        ...
        const SENTINEL: Tile = Tile::new(u16::MAX, 0, 0);

        // The previous tile visited.
        let mut prev_tile = tile_row.tiles[0];
        ...
        for (tile_idx, tile) in tile_row.tiles.iter().copied().chain([SENTINEL]).enumerate() {
            ...
            if !prev_tile.same_loc(&tile) && !prev_tile.prev_loc(&tile) {
                ...
                let is_sentinel = tile_idx == tile_row.tiles.len();
                if is_sentinel {
                    break;
                }
            ...
            if tile.x == 0 && line_left_x < 0. {
                ...
                for x_idx in 0..Tile::WIDTH {
                    location_winding[x_idx as usize] =
                        h.madd(sign, location_winding[x_idx as usize]);
                }
        ...
        if winding_delta != 0 || is_last_tile_row {
