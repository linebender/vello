...
impl Pixmap {
    ...
    pub fn from_parts(
        ...
        let data: Vec<PremulRgba8> = bytemuck::cast_vec(data);
        assert_eq!(
            data.len(),
...
mod tests {
    ...
    fn straight_alpha_is_premultiplied_in_body_and_tail() {
        let pixmap = Pixmap::from_parts(
            vec![
                // SIMD body
                200, 100, 50, 128, 128, 64, 32, 128, 255, 128, 64, 64, 255, 100, 1, 0, // 0-3
                64, 32, 16, 192, 10, 20, 30, 255, 240, 120, 60, 128, 80, 40, 20, 64, // 4-7
                100, 50, 25, 128, 32, 16, 8, 192, 200, 150, 100, 64, 3, 2, 1, 128, // 8-11
                254, 253, 252, 128, 1, 2, 3, 64, 127, 63, 31, 192, 9, 8, 7, 255, // 12-15
                // Scalar tail
                80, 40, 20, 64,
            ],
            17,
        ...
        assert_eq!(
            ...
            [
                // SIMD body
                100, 50, 25, 128, 64, 32, 16, 128, 64, 32, 16, 64, 0, 0, 0, 0, // 0-3
                48, 24, 12, 192, 10, 20, 30, 255, 120, 60, 30, 128, 20, 10, 5, 64, // 4-7
                50, 25, 13, 128, 24, 12, 6, 192, 50, 38, 25, 64, 2, 1, 1, 128, // 8-11
                127, 127, 126, 128, 1, 1, 1, 64, 96, 48, 24, 192, 9, 8, 7, 255, // 12-15
                // Scalar tail
                20, 10, 5, 64,
            ]
        );
    ...
    fn straight_opaque_alpha_clears_transparency_hint_in_body_and_tail() {
        let data = vec![
            // SIMD body
            200, 100, 50, 255, 1, 2, 3, 255, 4, 5, 6, 255, 7, 8, 9, 255, // 0-3
            10, 11, 12, 255, 13, 14, 15, 255, 16, 17, 18, 255, 19, 20, 21, 255, // 4-7
            22, 23, 24, 255, 25, 26, 27, 255, 28, 29, 30, 255, 31, 32, 33, 255, // 8-11
            34, 35, 36, 255, 37, 38, 39, 255, 40, 41, 42, 255, 43, 44, 45, 255, // 12-15
            // Scalar tail
            80, 40, 20, 255,
        ];
        let pixmap = Pixmap::from_parts(
