...
pub struct Pixmap {
    ...
    buf: Vec<PremulRgba8>,
    /// Whether the pixmap has any non-opaque pixels.
    ///
...
impl Pixmap {
    ...
    /// [`data_as_u8_slice_mut()`](Self::data_as_u8_slice_mut), or [`set_pixel()`](Self::set_pixel).
    ///
    /// Use [`set_has_opacities()`](Self::set_has_opacities) to manually update the flag,
    /// or [`recompute_has_opacities()`](Self::recompute_has_opacities) to recalculate it
    /// by scanning all pixels.
    pub fn has_opacities(&self) -> bool {
        self.has_opacities
    }

    /// Manually set the `has_opacities` flag.
    ///
    ...
    /// non-opaque pixels.
    pub fn set_has_opacities(&mut self, has_opacities: bool) {
        self.has_opacities = has_opacities;
    }

    /// Recalculate `has_opacities` by scanning all pixels.
    ///
    /// Use this after modifying pixels via [`data_mut()`](Self::data_mut) or
    /// [`set_pixel()`](Self::set_pixel) when you need accurate opacity information.
    pub fn recompute_has_opacities(&mut self) {
        self.has_opacities = self.buf.iter().any(|pixel| pixel.a != 255);
    }
    ...
    pub fn from_png(data: impl std::io::Read) -> Result<Self, png::DecodingError> {
        ...
        pixmap.has_opacities = has_opacities;
