...
pub struct CommandRecorder<D> {
    ...
    /// Flat storage for all draw commands referenced by recorded batches.
    pub draws: Vec<D>,
...
impl<D> CommandRecorder<D> {
    ...
    fn push_draw_batch(&mut self, draw_idx: u32) {
        match self.active_cmds_mut().last_mut() {
            Some(RecordedCmd::Batch(range)) if range.end == draw_idx => {
                range.end += 1;
            }
            _ => {
                self.push_render_cmd(RecordedCmd::Batch(draw_idx..draw_idx + 1));
            }
        };
    }
...
impl<D: Drawable> CommandRecorder<D> {
    ...
    #[inline]
    pub fn push_draw(&mut self, draw: D) {
        self.record_bbox(|| draw.bbox());
    ...
    fn assert_cmds(cmds: &[RecordedCmd], expected: &[ExpectedCmd]) {
        ...
        for (cmd, expected) in cmds.iter().zip(expected) {
            match (cmd, expected) {
                ...
                (RecordedCmd::Batch(range), ExpectedCmd::Batch(expected_range)) => {
