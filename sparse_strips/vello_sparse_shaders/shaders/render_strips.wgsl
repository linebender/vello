...
//
//
// `StripInstance::paint` field encodes a paint type and a paint texture id
...
//
// `StripInstance::payload` field can either encode a color or [x, y] for image sampling.
// - If the paint type is solid, the fragment shader uses the color directly.
// - If the paint type is image, the fragment shader samples the image.
// More details in the `StripInstance` documentation below.
...
//   payload               | Color / scene coords              | Color / scene coords
...
struct VertexOutput {
    ...
    // Color value for solid paints or sample coordinates for other paints.
