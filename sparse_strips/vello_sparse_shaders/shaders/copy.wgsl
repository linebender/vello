...
struct CopyInstance {
    @location(0) dest_texture_origin: u32,
    @location(1) source_texture_origin: u32,
    @location(2) copy_rect_size: u32,
    @location(3) dest_texture_size: u32,
}
...
fn vs_main(
    ...
    let ndc_x = dest_texture_xy.x * 2.0 / f32(dest_texture_size.x) - 1.0;
    let ndc_y = 1.0 - dest_texture_xy.y * 2.0 / f32(dest_texture_size.y);
