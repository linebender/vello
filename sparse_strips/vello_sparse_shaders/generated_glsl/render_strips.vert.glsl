...
layout(std140) uniform Config_block_0Vertex { Config _group_0_binding_1_vs; };

uniform highp usampler2D _group_2_binding_0_vs;
...
flat out uint _vs2fs_location3;
flat out uint _vs2fs_location4;
flat out uint _vs2fs_location5;

uvec2 encoded_paint_coord(uint flat_idx) {
    uint _e4 = _group_0_binding_1_vs.encoded_paints_tex_width_bits;
    uint _e11 = _group_0_binding_1_vs.encoded_paints_tex_width_bits;
...
EncodedImage unpack_encoded_image(uint paint_tex_idx) {
    ...
    uvec4 texel0_ = texelFetch(_group_2_binding_0_vs, ivec2(_e2), 0);
    uvec2 _e8 = encoded_paint_coord((paint_tex_idx + 1u));
    uvec4 texel1_ = texelFetch(_group_2_binding_0_vs, ivec2(_e8), 0);
    uvec2 _e14 = encoded_paint_coord((paint_tex_idx + 2u));
    uvec4 texel2_ = texelFetch(_group_2_binding_0_vs, ivec2(_e14), 0);
...
LinearGradient unpack_linear_gradient(uint paint_tex_idx_1) {
    ...
    uvec4 texel0_1 = texelFetch(_group_2_binding_0_vs, ivec2(_e2), 0);
    uvec2 _e8 = encoded_paint_coord((paint_tex_idx_1 + 1u));
    uvec4 texel1_1 = texelFetch(_group_2_binding_0_vs, ivec2(_e8), 0);
...
RadialGradient unpack_radial_gradient(uint paint_tex_idx_2) {
    ...
    uvec4 texel0_2 = texelFetch(_group_2_binding_0_vs, ivec2(_e2), 0);
    uvec2 _e8 = encoded_paint_coord((paint_tex_idx_2 + 1u));
    uvec4 texel1_2 = texelFetch(_group_2_binding_0_vs, ivec2(_e8), 0);
    uvec2 _e14 = encoded_paint_coord((paint_tex_idx_2 + 2u));
    uvec4 texel2_1 = texelFetch(_group_2_binding_0_vs, ivec2(_e14), 0);
    uvec2 _e20 = encoded_paint_coord((paint_tex_idx_2 + 3u));
    uvec4 texel3_ = texelFetch(_group_2_binding_0_vs, ivec2(_e20), 0);
...
SweepGradient unpack_sweep_gradient(uint paint_tex_idx_3) {
    ...
    uvec4 texel0_3 = texelFetch(_group_2_binding_0_vs, ivec2(_e2), 0);
    uvec2 _e8 = encoded_paint_coord((paint_tex_idx_3 + 1u));
    uvec4 texel1_3 = texelFetch(_group_2_binding_0_vs, ivec2(_e8), 0);
    uvec2 _e14 = encoded_paint_coord((paint_tex_idx_3 + 2u));
    uvec4 texel2_2 = texelFetch(_group_2_binding_0_vs, ivec2(_e14), 0);
...
void main() {
    ...
    StripInstance instance = StripInstance(_p2vs_location0, _p2vs_location1, _p2vs_location2, _p2vs_location3, _p2vs_location4, _p2vs_location5);
    VertexOutput out_ = VertexOutput(0u, vec2(0.0), vec2(0.0), 0u, 0u, 0u, vec4(0.0));
    uint height = 0u;
    bool local = false;
    bool local_1 = false;
    float x_2 = float((in_vertex_index & 1u));
    float y_1 = float((in_vertex_index >> 1u));
    uint x0_ = (instance.xy & 65535u);
    uint y0_ = (instance.xy >> 16u);
    uint width = (instance.widths_or_rect_height & 65535u);
    uint dense_width = (instance.widths_or_rect_height >> 16u);
    bool is_rect = ((instance.paint_and_rect_flag & RECT_STRIP_FLAG) != 0u);
    uint _e28 = _group_0_binding_1_vs.strip_height;
    height = _e28;
    if (is_rect) {
        height = dense_width;
        out_.dense_end_or_rect_size = (width | (dense_width << 16u));
        out_.rect_frac = instance.col_idx_or_rect_frac;
    } else {
        out_.dense_end_or_rect_size = (instance.col_idx_or_rect_frac + dense_width);
        out_.rect_frac = 0u;
    }
    int _e44 = _group_0_binding_1_vs.strip_offset_x;
    float pix_x = (float((int(x0_) + _e44)) + (x_2 * float(width)));
    int _e53 = _group_0_binding_1_vs.strip_offset_y;
    uint _e56 = height;
    float pix_y = (float((int(y0_) + _e53)) + (y_1 * float(_e56)));
    uint _e64 = _group_0_binding_1_vs.width;
    float ndc_x = (((pix_x * 2.0) / float(_e64)) - 1.0);
    uint _e73 = _group_0_binding_1_vs.height;
    float ndc_y = (1.0 - ((pix_y * 2.0) / float(_e73)));
    uint color_source = ((instance.paint_and_rect_flag >> 29u) & 3u);
    if ((color_source == COLOR_SOURCE_PAYLOAD)) {
        uint paint_type = ((instance.paint_and_rect_flag >> 26u) & 7u);
        uint scene_strip_x = (instance.payload & 65535u);
        uint scene_strip_y = (instance.payload >> 16u);
        if ((paint_type == PAINT_TYPE_IMAGE)) {
            uint paint_tex_idx_4 = (instance.paint_and_rect_flag & PAINT_TEXTURE_INDEX_MASK);
            EncodedImage _e101 = unpack_encoded_image(paint_tex_idx_4);
            uint _e107 = height;
            vec2 pos = vec2((float(scene_strip_x) + (x_2 * float(width))), (float(scene_strip_y) + (y_1 * float(_e107))));
            out_.sample_xy = ((_e101.translate + _e101.image_offset) + (_e101.transform * pos));
        } else {
            if (!((paint_type == PAINT_TYPE_LINEAR_GRADIENT))) {
                local = (paint_type == PAINT_TYPE_RADIAL_GRADIENT);
            } else {
                local = true;
            }
            bool _e127 = local;
            if (!(_e127)) {
                local_1 = (paint_type == PAINT_TYPE_SWEEP_GRADIENT);
            } else {
                local_1 = true;
            }
            bool _e134 = local_1;
            if (_e134) {
                uint _e141 = height;
                out_.sample_xy = vec2((float(scene_strip_x) + (x_2 * float(width))), (float(scene_strip_y) + (y_1 * float(_e141))));
            }
        }
    }
    float col_offset = (is_rect ? 0.0 : float(instance.col_idx_or_rect_frac));
    uint _e154 = height;
    out_.tex_coord = vec2((col_offset + (x_2 * float(width))), (y_1 * float(_e154)));
    float z = (1.0 - (float(instance.depth_index) / 16777216.0));
    uint _e167 = _group_0_binding_1_vs.ndc_y_negate;
    float final_ndc_y = ((_e167 != 0u) ? -(ndc_y) : ndc_y);
    out_.position = vec4(ndc_x, final_ndc_y, z, 1.0);
    out_.payload = instance.payload;
    out_.paint_and_rect_flag = instance.paint_and_rect_flag;
    VertexOutput _e178 = out_;
    _vs2fs_location0 = _e178.paint_and_rect_flag;
    _vs2fs_location1 = _e178.tex_coord;
    _vs2fs_location2 = _e178.sample_xy;
    _vs2fs_location3 = _e178.dense_end_or_rect_size;
    _vs2fs_location4 = _e178.payload;
    _vs2fs_location5 = _e178.rect_frac;
    gl_Position = _e178.position;
