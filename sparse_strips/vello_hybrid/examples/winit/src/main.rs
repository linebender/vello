    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                // Fill the entire scene with white background
...
impl App<'_> {
    fn upload_images_to_atlas(&mut self, device_id: usize) {
        ...
        let images: Vec<Pixmap> = vec![
            ...
            ImageScene::read_desert_image(),
            ImageScene::read_cowboy_image(),
            ImageScene::read_rgb_image_10x10(),
        ];
        for image in images {
            let texture =
                self.upload_image_to_texture(&device_handle.device, &device_handle.queue, &image);
            let image_resource = self.image_cache.register_image(texture);
