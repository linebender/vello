...
//! [`Renderer`](glifo::Renderer) implementation for [`Scene`].
//!
...
use glifo::{
    AtlasCacher, AtlasSlot, DrawTarget, GLYPH_PADDING, GlyphAtlas, GlyphCacheConfig,
...
/// [`DrawTarget`] for the hybrid [`Scene`].
///
/// The `default_blending_only` blend-wrapping concern is now handled inside
/// `Scene::push_layer` / `Scene::pop_layer` directly, so no wrapper is needed.
