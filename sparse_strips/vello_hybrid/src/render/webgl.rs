...
impl LayerTargetPool {
    const MAX_RETAINED_TARGETS: usize = 48;
    const SIZE_BUCKET: u16 = 64;

    fn bucket_dimension(size: u16) -> u16 {
        let bucket = u32::from(Self::SIZE_BUCKET);
        let size = u32::from(size.max(1));
        size.div_ceil(bucket)
    ...
    fn acquire(
        ...
        if let Some(index) = self
            ...
            .iter()
            .enumerate()
            .filter(|(_, target)| target.texture_width >= width && target.texture_height >= height)
            .min_by_key(|(_, target)| {
                u32::from(target.texture_width) * u32::from(target.texture_height)
            })
            .map(|(index, _)| index)
        {
    ...
    fn release(&mut self, gl: &WebGl2RenderingContext, target: LayerTarget) {
        self.targets.push(target);
        if self.targets.len() > Self::MAX_RETAINED_TARGETS
            && let Some(index) = self
                .targets
                .iter()
                .enumerate()
                ...
                .map(|(index, _)| index)
        {
            ...
            gl.delete_texture(Some(&target.texture));
        }
    }
}
...
impl LayerTarget {
    fn new(
        ...
        let texture_width = LayerTargetPool::bucket_dimension(width);
        let texture_height = LayerTargetPool::bucket_dimension(height);
...
impl WebGlRenderer {
    ...
    ///
    /// This method creates GPU resources as needed and renders the direct strip stream.
    pub fn render(
        &mut self,
        scene: &Scene,
    ...
    pub fn render_to_atlas(
        ...
        // Set the view framebuffer override so the direct strip pass renders to the
    ...
    /// Shared render pipeline: prepares GPU resources, renders direct strips, and
    /// maintains caches.
    ///
    ...
    fn create_layer_target(
        ...
        let root_render_size = RenderSize {
            width: u32::from(scene.width),
            height: u32::from(scene.height),
        };
        let (origin, render_size) =
            layer_target_origin_and_size(scene, layer_idx, &root_render_size);
        self.create_layer_target_with_size(
            texture_id,
            render_size.width.try_into().unwrap_or(u16::MAX),
            render_size.height.try_into().unwrap_or(u16::MAX),
    ...
    fn release_layer_target(&mut self, target: LayerTarget) {
        self.layer_target_pool.release(&self.gl, target);
    }
    ...
    fn upload_layer_filter_data(&mut self, gpu_filters: &[GpuFilterData]) {
        ...
        self.gl.bind_texture(
            WebGl2RenderingContext::TEXTURE_2D,
            Some(&self.programs.resources.layer_filter_data_texture),
        );
        self.gl
...
struct WebGlResources {
    ...
    filter_instance_buffer_capacity: usize,
...
impl WebGlPrograms {
    ...
    fn maybe_resize_filter_data_tex(
        ...
        if required_height > self.resources.filter_data_texture_height {
            ...
            allocate_rgba32ui_texture(gl, max_texture_dimension_2d, required_height);
        }
    }
    ...
    fn upload_filter_data_texture(
        ...
        upload_rgba32ui_sub_texture(gl, &packed_array, width, height);
    }
    ...
    fn grow_buffer_capacity(required_size: usize) -> usize {
        required_size.next_power_of_two().max(256)
    }

    fn ensure_filter_instance_buffer_capacity(
        &mut self,
        gl: &WebGl2RenderingContext,
        required_size: usize,
    ) {
    ...
    fn upload_filter_instances(
        ...
        self.ensure_filter_instance_buffer_capacity(gl, instance_data.len());
        gl.bind_buffer(
            WebGl2RenderingContext::ARRAY_BUFFER,
            Some(&self.resources.filter_instance_buffer),
        );
    ...
    fn maybe_resize_alphas_tex(
        ...
        if required_alpha_height > current_alpha_height {
            ...
            allocate_rgba32ui_texture(gl, max_texture_dimension_2d, required_alpha_height);
        }
    }
    ...
    fn maybe_resize_encoded_paints_tex(
        ...
        if required_encoded_paints_height > current_encoded_paints_height {
            ...
            allocate_rgba32ui_texture(gl, max_texture_dimension_2d, required_encoded_paints_height);
        }
    }
    ...
    fn maybe_resize_gradient_tex(
        ...
        if required_gradient_height > current_gradient_height {
            ...
            allocate_rgba8_texture(gl, max_texture_dimension_2d, required_gradient_height);
        }
    }
    ...
    fn upload_alpha_texture(&mut self, gl: &WebGl2RenderingContext, alphas: &mut Vec<u8>) {
        ...
        let packed_array =
            unsafe { js_sys::Uint32Array::view(bytemuck::cast_slice::<u8, u32>(alphas)) };
        upload_rgba32ui_sub_texture(gl, &packed_array, alpha_texture_width, alpha_texture_height);
    ...
    fn upload_encoded_paints_texture(
        ...
        if !encoded_paints.is_empty() {
            ...
            let packed_array = unsafe {
                js_sys::Uint32Array::view(bytemuck::cast_slice::<u8, u32>(
                    &self.encoded_paints_data,
                ))
            };
            upload_rgba32ui_sub_texture(
                gl,
                &packed_array,
    ...
    fn upload_gradient_texture(
        ...
        let packed_array = unsafe { js_sys::Uint8Array::view(&luts) };
...
fn create_layer_filter_data_texture(gl: &WebGl2RenderingContext) -> WebGlTexture {
    let texture = create_texture(gl);
    gl.tex_image_2d_with_i32_and_i32_and_i32_and_format_and_type_and_opt_array_buffer_view(
        WebGl2RenderingContext::TEXTURE_2D,
        0,
...
fn create_webgl_resources(
    ...
    let layer_filter_data_texture = create_layer_filter_data_texture(gl);
...
impl WebGlRendererContext<'_> {
    ...
    fn do_strip_render_pass(
        ...
        *self.render_pass_count += 1;
        ...
        self.gl.bind_texture(
            WebGl2RenderingContext::TEXTURE_2D,
            Some(&self.programs.resources.slot_textures[1]),
        );
        self.gl
    ...
    fn do_clear_slots_render_pass(&mut self, ix: usize, slot_indices: &[u32]) {
        ...
        self.gl
            .viewport(0, 0, SLOT_TEXTURE_WIDTH as i32, height as i32);
...
fn allocate_rgba32ui_texture(gl: &WebGl2RenderingContext, width: u32, height: u32) {
    gl.tex_image_2d_with_i32_and_i32_and_i32_and_format_and_type_and_opt_array_buffer_view(
        WebGl2RenderingContext::TEXTURE_2D,
        0,
        WebGl2RenderingContext::RGBA32UI as i32,
...
fn allocate_rgba8_texture(gl: &WebGl2RenderingContext, width: u32, height: u32) {
    gl.tex_image_2d_with_i32_and_i32_and_i32_and_format_and_type_and_opt_array_buffer_view(
        WebGl2RenderingContext::TEXTURE_2D,
        0,
        WebGl2RenderingContext::RGBA8 as i32,
...
fn upload_rgba32ui_sub_texture(
    gl: &WebGl2RenderingContext,
    ...
    // The `tex_sub_image_2d` method is synchronous in the sense that once it returns, it is guaranteed
