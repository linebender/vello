...
impl WebGlRenderer {
    ...
    pub fn render_to_atlas(&mut self, scene: &Scene, atlas_id: AtlasId) -> Result<(), RenderError> {
        ...
        let atlas_framebuffer = self.gl.create_framebuffer().unwrap();
        self.gl.bind_framebuffer(
        ...
        // Run the scheduler targeting the atlas layer.
        let mut ctx = WebGlRendererContext {
            programs: &mut self.programs,
            gl: &self.gl,
        };
        ...
        // Restore the original view framebuffer.
        let atlas_fb = core::mem::replace(
            &mut self.programs.resources.view_framebuffer,
            saved_framebuffer,
        );
        self.gl.delete_framebuffer(Some(&atlas_fb));
