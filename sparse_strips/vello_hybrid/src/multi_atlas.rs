...
impl MultiAtlasManager {
    ...
    fn allocate_first_fit(
        ...
        for atlas in &mut self.atlases {
            if Some(atlas.id) == exclude_atlas {
                continue;
            }
    ...
    fn allocate_best_fit(
        ...
        self.allocate_first_fit(width, height, exclude_atlas)
    }
    ...
    fn allocate_round_robin(
        ...
        if self.atlases.is_empty() {
            return self.allocate_first_fit(width, height, exclude_atlas);
        }
        ...
        for i in 0..self.atlases.len() {
            ...
            if Some(self.atlases[idx].id) == exclude_atlas {
                continue;
            }
