...
use vello_api::{
    PaintScene, Renderer, SceneOptions,
    peniko::Color,
    texture::{TextureDescriptor, TextureUsages},
};
use vello_cpu::{
...
pub(crate) fn run_test_cpu<M>(
    ...
    let renderer = VelloCPU::new(render_settings);
    ...
    if !params.no_ref {
        let pixmap = renderer.read_texture(target.id()).unwrap();
        check_pixmap_ref(
            params.name,
            specific_name,
            threshold,
...
pub(crate) fn run_test_hybrid_wgpu<M>(
    ...
    let wgpu_texture = renderer.wgpu_texture(target.id()).unwrap();
    if params.no_ref {
        // Make sure the GPU work completes.
        device.poll(wgpu::PollType::wait_indefinitely()).unwrap();
    } else {
        // Obviously in the future, we'd like this to reuse the same command encoder somehow
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("Vello Copy to Buffer"),
        });
...
// Marker types used to disambiguate implementations of SceneFunction.
...
where
    F: Fn(&mut S),
{
    fn run(self, scene: &mut S, _: &dyn Renderer) {
        self(scene);
    }
}
