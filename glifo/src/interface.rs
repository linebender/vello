// Copyright 2026 the Vello Authors
// SPDX-License-Identifier: Apache-2.0 OR MIT

//! Traits for rendering glyphs and replaying them.

use crate::atlas::{AtlasPaint, AtlasSlot};
use crate::color::{AlphaColor, Srgb};
use crate::kurbo::{Affine, BezPath, Rect};
use crate::peniko::BlendMode;
use vello_common::paint::{Image, ImageSource, Tint};

/// A sink for low-level glyph drawing commands.
pub trait DrawSink {
    /// Set the current transform.
    fn set_transform(&mut self, t: Affine);
    /// Set the current paint.
    fn set_paint(&mut self, paint: AtlasPaint);
    /// Set the paint transform.
    fn set_paint_transform(&mut self, t: Affine);
    /// Fill a path with the current paint and transform.
    fn fill_path(&mut self, path: &BezPath);
    /// Fill a rectangle with the current paint and transform.
    fn fill_rect(&mut self, rect: &Rect);
    /// Push a clip layer defined by a path.
    fn push_clip_layer(&mut self, clip: &BezPath);
    /// Push a blend/compositing layer.
    fn push_blend_layer(&mut self, blend_mode: BlendMode);
    /// Pop the most recent clip or blend layer.
    fn pop_layer(&mut self);
    /// Width of the surface.
    fn width(&self) -> u16;
    /// Height of the surface.
    fn height(&self) -> u16;
    // This method is only needed as a (hopefully) temporary workaround for vello_hybrid.
    /// Whether COLR glyphs with non-default blending need to be wrapped in a
    /// layer.
    fn needs_root_blend_wrapper(&self) -> bool {
        false
    }
}

// See this PR for a bit more context on why we have this.
// https://github.com/linebender/vello/pull/1554
// In short, we want to ensure that COLR rendering works even when the
// `default_blending_only` scene constraint is enabled in vello_hybrid. In order to do so,
// we need to ensure that we don't use non-default blending to blend into
// the root layer.
pub(crate) struct RootBlendWrapper<'a, T: DrawSink + ?Sized> {
    target: &'a mut T,
    layer_depth: usize,
    inserted_root_wrapper: bool,
    wrap_root_blends: bool,
}

impl<'a, T: DrawSink + ?Sized> RootBlendWrapper<'a, T> {
    pub(crate) fn new(target: &'a mut T) -> Self {
        Self {
            wrap_root_blends: target.needs_root_blend_wrapper(),
            target,
            layer_depth: 0,
            inserted_root_wrapper: false,
        }
    }
}

impl<T: DrawSink + ?Sized> DrawSink for RootBlendWrapper<'_, T> {
    #[inline]
    fn set_transform(&mut self, t: Affine) {
        self.target.set_transform(t);
    }

    #[inline]
    fn set_paint(&mut self, paint: AtlasPaint) {
        self.target.set_paint(paint);
    }

    #[inline]
    fn set_paint_transform(&mut self, t: Affine) {
        self.target.set_paint_transform(t);
    }

    #[inline]
    fn fill_path(&mut self, path: &BezPath) {
        self.target.fill_path(path);
    }

    #[inline]
    fn fill_rect(&mut self, rect: &Rect) {
        self.target.fill_rect(rect);
    }

    #[inline]
    fn push_clip_layer(&mut self, clip: &BezPath) {
        self.target.push_clip_layer(clip);
        self.layer_depth += 1;
    }

    #[inline]
    fn push_blend_layer(&mut self, blend_mode: BlendMode) {
        if self.wrap_root_blends && self.layer_depth == 0 && blend_mode != BlendMode::default() {
            self.target.push_blend_layer(BlendMode::default());
            self.inserted_root_wrapper = true;
        }

        self.target.push_blend_layer(blend_mode);
        self.layer_depth += 1;
    }

    #[inline]
    fn pop_layer(&mut self) {
        self.target.pop_layer();
        self.layer_depth -= 1;

        if self.layer_depth == 0 && self.inserted_root_wrapper {
            self.target.pop_layer();
            self.inserted_root_wrapper = false;
        }
    }

    #[inline]
    fn width(&self) -> u16 {
        self.target.width()
    }

    #[inline]
    fn height(&self) -> u16 {
        self.target.height()
    }
}

/// A stateful renderer that can draw sequences of cached and uncached glyphs.
pub trait GlyphRenderer: DrawSink {
    /// The type of state used by the renderer.
    type SavedState;

    /// Save the current state.
    fn save_state(&mut self) -> Self::SavedState;

    /// Restore the current state.
    fn restore_state(&mut self, state: Self::SavedState);

    /// Stroke a path with the current paint and stroke settings.
    fn stroke_path(&mut self, path: &BezPath);

    /// Set the current paint to an image.
    fn set_paint_image(&mut self, image: Image);

    /// Set the tint for subsequent image draws.
    fn set_tint(&mut self, tint: Option<Tint>);

    /// Get the context color from the renderer's current paint, used for resolving the
    /// context-dependent colors of COLR glyphs.
    fn get_context_color(&self) -> AlphaColor<Srgb>;

    // Hopefully we can get rid of those below in the future.

    /// Construct the [`ImageSource`] for sampling a cached glyph from the atlas.
    fn atlas_image_source(&self, atlas_slot: &AtlasSlot) -> ImageSource;

    /// Compute the paint transform for sampling a cached glyph from the atlas.
    fn atlas_paint_transform(&self, atlas_slot: &AtlasSlot) -> Affine;
}
