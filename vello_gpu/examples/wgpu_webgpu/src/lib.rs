...
//! Demonstrates using Vello GPU through wgpu's WebGPU backend in the browser.
...
impl AppState {
    async fn new(canvas: HtmlCanvasElement, scenes: Box<[AnyScene<Scene>]>) -> Self {
        ...
        let mut app_state = Self {
            ...
            performance: PerformancePanel::new(
                ...
                [
                    ...
                    PerformanceStage {
                        label: "Render/encode",
                        description: "CPU time to encode the renderer's WebGPU commands.",
                        color: "#f59e0b",
                    },
                    PerformanceStage {
                ...
                "GPU timing is unavailable for the WebGPU demo",
            ),
            canvas,
        };
        ...
        // Upload images to the renderer's texture atlas.
    ...
    fn update_title(&self) {
        web_sys::window()
            ...
            .unwrap()
            .set_title(&format!(
                "Vello GPU WebGPU via wgpu - Page {}/{}",
                self.current_scene + 1,
                self.scenes.len()
            ));
    }
    ...
    fn upload_images_to_atlas(&mut self) {
        ...
        // 1st example — uploading a pixmap directly to the texture atlas.
        ...
        // 2nd example — uploading from a wgpu texture.
