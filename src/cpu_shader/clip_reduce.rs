...
pub fn clip_reduce(n_wg: u32, resources: &[CpuBinding]) {
    ...
    let path_bboxes = bytemuck::cast_slice_mut(r2.as_mut());
    let reduced = bytemuck::cast_slice_mut(r3.as_mut());
    let clip_out = bytemuck::cast_slice_mut(r4.as_mut());
    clip_reduce_main(n_wg, clip_inp, path_bboxes, reduced, clip_out);
}
