...
void main() {
    VertexOutput in_ = VertexOutput(_vs2fs_location0, _vs2fs_location1, _vs2fs_location2, _vs2fs_location3, _vs2fs_location4, _vs2fs_location5, gl_FragCoord);
    float alpha = 1.0;
    bool local_2 = false;
    bool local_3 = false;
    ...
    if (is_rect) {
        local_2 = (in_.rect_frac != 0u);
    } else {
        local_2 = false;
    }
    bool _e14 = local_2;
    if (_e14) {
        uint _e15 = in_.rect_frac;
        vec4 frac = (vec4(_e15 & 0xFFu, _e15 >> 8 & 0xFFu, _e15 >> 16 & 0xFFu, _e15 >> 24) / 255.0);
        vec2 rect_size = vec2(float((in_.dense_end_or_rect_size & 65535u)), float((in_.dense_end_or_rect_size >> 16u)));
        ...
        if (!(is_rect)) {
            local_3 = (in_.dense_end_or_rect_size != 0u);
        } else {
            local_3 = false;
        }
        bool _e54 = local_3;
        if (_e54) {
            ...
            uint y_1 = uint(floor(in_.tex_coord.y));
    ...
    if ((color_source == COLOR_SOURCE_PAYLOAD)) {
        ...
        if ((color_source == COLOR_SOURCE_SLOT)) {
            ...
            float sample_y = ((_e273 != 0u) ? (float(_e266) - in_.position.y) : in_.position.y);
            int _e282 = _group_0_binding_1_fs.strip_offset_x;
            uint clip_x = (uint((int(in_.position.x) - _e282)) & 255u);
            int _e290 = _group_0_binding_1_fs.strip_offset_y;
            uint _e298 = _group_0_binding_1_fs.strip_height;
            uint clip_y = ((uint((int(sample_y) - _e290)) & 3u) + (in_.payload * _e298));
            ...
            if ((color_source == COLOR_SOURCE_BLEND)) {
                ...
                float sample_y_1 = ((_e327 != 0u) ? (float(_e320) - in_.position.y) : in_.position.y);
                float opacity_1 = (float(((in_.paint_and_rect_flag >> 16u) & 255u)) * 0.003921569);
                uint mix_mode_1 = ((in_.paint_and_rect_flag >> 8u) & 255u);
                uint compose_mode_1 = (in_.paint_and_rect_flag & 255u);
                uint src_slot = (in_.payload & 65535u);
                uint dest_slot = ((in_.payload >> 16u) & 65535u);
                int _e360 = _group_0_binding_1_fs.strip_offset_x;
                uint clip_x_1 = (uint((int(in_.position.x) - _e360)) & 255u);
