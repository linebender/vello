...
// To make developing Vello without using sdl2 easier, we don't enable sdl2 unless
// the sdl2 feature is enabled
...
fn main() {
    let mut command_suffix = std::env::args().skip(1).collect::<Vec<_>>().join(" ");
    if !command_suffix.is_empty() {
        command_suffix.insert_str(0, " --");
    }
    if !cfg!(debug_assertions) {
        command_suffix.insert_str(0, " --release");
    }
    panic!(
        "To run the sdl2 example, rerun using `--features sdl2`.\n\
    That is, `cargo run -p simple_sdl2 --features sdl2{command_suffix}`"
    )
}
