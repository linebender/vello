...
fn flatten_euler(
    ...
    let (p0, p1, p2, p3, scale, transform) = if start_offset == 0. && end_offset == 0. {
        (
    ...
    let (t_start, t_end) = if start_offset == 0. && end_offset == 0. {
        (p0, p3)
    } else {
        (start_p, end_p)
    };
    ...
    loop {
        ...
        if cubic_params.err * scale <= tol || dt <= SUBDIV_LIMIT {
            ...
            // Interpolate offset at the start of the current segment (t0 relative to the whole curve)
            let t_ratio_start = t0; // or last_t if you want to use the actual t value
            let normalized_offset_at_start = normalized_offset_start * (1.0 - t_ratio_start)
                + normalized_offset_end * t_ratio_start;

            // Interpolate offset at the end of the current segment
            let t_ratio_end = t1;
            let normalized_offset_at_end =
                normalized_offset_start * (1.0 - t_ratio_end) + normalized_offset_end * t_ratio_end;

            // Use average of the interpolated values for this segment
            let normalized_offset_avg =
                0.5 * (normalized_offset_at_start + normalized_offset_at_end);
            ...
            for i in 0..n as usize {
                let lp1 = if i == n as usize - 1 && t1 == 1.0 {
                    ...
                    let normalized_offset_t =
                        normalized_offset_start * (1.0 - s) + normalized_offset_end * s;
                    es.eval_with_offset(s, normalized_offset_t)
                };
                let l0 = if start_offset >= 0. { lp0 } else { lp1 };
                let l1 = if start_offset >= 0. { lp1 } else { lp0 };
...
trait VecExt {
    fn project_abs(&self, other: Self) -> f32;
}
impl VecExt for Vec2 {
    fn project_abs(&self, tangent: Self) -> f32 {
        let tan_n = tangent.normalize();
        self.x * tan_n.y.abs() + self.y * tan_n.x.abs()
    }
}
    ...
    for ix in 0..n_wg as usize * WG_SIZE {
        ...
        if seg_type != 0 {
            ...
            if is_stroke {
                ...
                let embolden_y =
                    f32::from_bits(scene[(config.layout.style_base + style_ix + 3) as usize]);
                let embolden = Vec2::new(embolden_x, embolden_y);
                ...
                if is_stroke_cap_marker {
                    if is_open {
                        ...
                        let embolden = embolden.project_abs(tangent);
                    ...
                    let start_embolden = embolden.project_abs(tan_start);
                    let start_offset = 0.5 * (linewidth + start_embolden);
                    let n_start = start_offset * Vec2::new(-tan_start.y, tan_start.x).normalize();

                    let prev_embolden = embolden.project_abs(tan_prev);
                    let prev_offset = 0.5 * (linewidth + prev_embolden);
                    let offset_tangent = prev_offset * tan_prev.normalize();
                    let n_prev = Vec2::new(-offset_tangent.y, offset_tangent.x);

                    let next_embolden = embolden.project_abs(tan_next);
                    let next_offset = 0.5 * (linewidth + next_embolden);
                    let n_next = next_offset * Vec2::new(-tan_next.y, tan_next.x).normalize();
                ...
                if (embolden_x != 0. || embolden_y != 0.) && !is_stroke_cap_marker {
                    let sign =
                    ...
                    let start_embolden = embolden.project_abs(tan_start);
                    let n_start =
                        -start_embolden * Vec2::new(-tan_start.y, tan_start.x).normalize();

                    let prev_embolden = embolden.project_abs(tan_prev);
                    let mut n_prev =
                        -prev_embolden * Vec2::new(-tan_prev.y, tan_prev.x).normalize();
                    ...
                    if neighbor.do_join {
                        ...
                        let mut tan_next = if tan_next.length_squared() < TANGENT_THRESH.powi(2) {
                            Vec2::new(TANGENT_THRESH, 0.)
                        } else {
                            tan_next
                        };
                        let next_embolden = embolden.project_abs(tan_next);
                        let mut n_next =
                            -next_embolden * Vec2::new(-tan_next.y, tan_next.x).normalize();
                        if sign < 0. {
...
pub fn flatten(n_wg: u32, resources: &[CpuBinding<'_>]) {
    ...
    // println!("\n\n\nnew");
    flatten_with_svg_debug(n_wg, resources, "test_no_join.svg").unwrap();
}
...
// use super::{IntBbox, LineSoup, Transform, Vec2};

/// Converts a flattened path to SVG format for debugging purposes
struct SvgDebugOutput {
    file: File,
    // width: i32,
    // height: i32,
    // scale: f32,
...
impl SvgDebugOutput {
    /// Creates a new SVG debug output file
    fn new<P: AsRef<Path>>(path: P, bbox: &IntBbox, padding: i32) -> io::Result<Self> {
        let width = (bbox.x1 - bbox.x0) + padding * 2;
        let height = (bbox.y1 - bbox.y0) + padding * 2;
        ...
        // writeln!(file, r##"  <g id="grid" stroke="#eee" stroke-width="0.5">"##)?;
        // let grid_size = 10;
        // for x in ((bbox.x0 - padding) / grid_size * grid_size..=(bbox.x1 + padding)).step_by(grid_size as usize) {
        //     writeln!(file, r#"    <line x1="{}" y1="{}" x2="{}" y2="{}" />"#, x, bbox.y0 - padding, x, bbox.y1 + padding)?;
        // }
        // for y in ((bbox.y0 - padding) / grid_size * grid_size..=(bbox.y1 + padding)).step_by(grid_size as usize) {
        //     writeln!(file, r#"    <line x1="{}" y1="{}" x2="{}" y2="{}" />"#, bbox.x0 - padding, y, bbox.x1 + padding, y)?;
        // }
        // writeln!(file, r#"  </g>"#)?;
        ...
        Ok(Self {
            ...
            colors: vec![
                ...
                "#17becf".to_string(), // teal
            ],
            current_color_idx: 0,
        })
    }

    /// Writes a collection of lines to the SVG file
    fn add_lines(&mut self, lines: &[LineSoup], transform: &Transform) -> io::Result<()> {
        let color = &self.colors[self.current_color_idx];
        self.current_color_idx = (self.current_color_idx + 1) % self.colors.len();

        let mut current_path_ix = u32::MAX;
        ...
        for line in lines {
            // Start a new path if the path index changes
            if line.path_ix != current_path_ix {
                if path_open {
                    writeln!(self.file, r#"    "/>"#)?;
                }

                current_path_ix = line.path_ix;
                writeln!(self.file, r#"    <path d=""#)?;
                path_open = true;

                // Convert first point
                let p0 = Vec2::new(line.p0[0], line.p0[1]);
                let transformed_p0 = transform.apply(p0);
                write!(self.file, "M{},{} ", transformed_p0.x, transformed_p0.y)?;
            }

            // Add the line segment
            let p0 = Vec2::new(line.p0[0], line.p0[1]);
            let p1 = Vec2::new(line.p1[0], line.p1[1]);
            let transformed_p0 = transform.apply(p0);
            let transformed_p1 = transform.apply(p1);
            write!(self.file, "M{},{} ", transformed_p0.x, transformed_p0.y)?;
            write!(self.file, "L{},{} ", transformed_p1.x, transformed_p1.y)?;
        }
        ...
        writeln!(self.file, r#"  </g>"#)?;

        Ok(())
    }

    /// Finalizes and closes the SVG file
    fn finalize(mut self) -> io::Result<()> {
        writeln!(self.file, "</svg>")?;
        Ok(())
    }
}

// Helper function to output SVG for debugging
fn debug_to_svg(lines: &[LineSoup], path: &str) -> io::Result<()> {
    // Calculate bounding box
    let mut bbox = IntBbox::default();
    for line in lines {
        bbox.add_pt(Vec2::new(line.p0[0], line.p0[1]));
        bbox.add_pt(Vec2::new(line.p1[0], line.p1[1]));
    }

    let mut svg = SvgDebugOutput::new(path, &bbox, 10)?;
    svg.add_lines(lines, &Transform::identity())?;
    svg.finalize()
}

// Add this to the `flatten` function
fn flatten_with_svg_debug(
    n_wg: u32,
    ...
    // Run the normal flattening algorithm
    flatten_main(
        n_wg,
        &config,
        &scene,
    ...
    // Output SVG with the generated lines
    let lines_count = bump.lines as usize;
    debug_to_svg(&lines[0..lines_count], svg_path)
}
